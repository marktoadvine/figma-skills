---
name: better-accessibility-figma
description: Audits a selected Figma frame, component, or multi-screen user flow against WCAG 2.2 AA using measured values, marks up the canvas with annotated fixes, and emits a pa11y config so the criteria a static design cannot prove are verified in CI. Use this skill whenever the user mentions accessibility, a11y, WCAG, AA/AAA, contrast, colour-blindness, screen readers, pa11y, axe, RGAA, ADA, Section 508, or the Equality Act in relation to a Figma file, frame, flow, or component. Also use it when the user asks to annotate, mark up, or review a design for accessibility, asks for a design-system audit or handoff check, or says something like "is this usable / is this readable / will this pass" about anything on the Figma canvas — even if they never say the word "accessibility."
---
...
**In Figma's agent** (upload this file as a custom skill): nothing else to set up. It
audits the current selection and writes to the current file.

**In an MCP client** (Claude Code, Cursor): load `figma-use` first and pass it in
`skillNames` on every `use_figma` call, alongside `better-accessibility-figma`. If Figma
tools appear as deferred tools, batch-load them in one call:
`ToolSearch query="select:use_figma,get_metadata,get_screenshot,search_design_system,whoami"`.

Writing to the canvas needs a **Full seat**. On a Dev or view seat, produce the report and
the pa11y config, and say plainly that the markup step was skipped — do not fail silently.
...
Written by Mark Toadvine for Figma agents, inspired by original work done by
jakubkrehel/skills.
